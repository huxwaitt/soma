...
def validate_output_file(raw: str, *, allowed_suffixes: tuple[str, ...] = ()) -> str:
    """Validate an output *file* path: absolute, under the user profile,
    with an allowed extension. Creates the parent directory."""
    if not raw:
        raise OutlookError("Output file path is empty.")
    p = Path(raw)
    if not p.is_absolute():
        raise OutlookError(f"Output file '{raw}' must be absolute.")
    if allowed_suffixes and p.suffix.lower() not in allowed_suffixes:
        raise OutlookError(
            f"Output file '{raw}' must end with one of: {', '.join(allowed_suffixes)}."
        )
    if not _sandbox_disabled() and not _within_profile(p):
        raise OutlookError(
            f"Output file '{raw}' is outside the user profile. Set "
            "OUTLOOK_MCP_ALLOW_ANY_PATH=1 to allow arbitrary paths."
        )
    if p.exists() and p.is_dir():
        raise OutlookError(f"Output file '{raw}' is a directory.")
