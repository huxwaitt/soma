...
Two servers ship from this checkout: `outlook-mcp`, which reads classic Outlook, and `administrator-vault`, which writes the Obsidian notes. The version here is the one the vault stamps on every note it writes as `created_by: administrator/<version>`.
...
- **Safety rails.** `vault_init` zips an existing `Administrator/` before the first call that rewrites a wiki written by an older version; the read tools only report hand edits and the next writing call adopts them; the Teams reader is pinned to a known commit.
...
- **`administrator-vault`.** A second server that writes the Obsidian notes, picks the filenames, checks the required keys and refuses a second note for an identity that already exists.
