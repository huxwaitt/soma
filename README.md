...
A page name shows the page (lead, facts, open items, newest records). A question is answered from the facts the wiki's own search ranks highest across every page, every claim with its page link. A statement becomes one operation on one page, shown first and written only after you say ok, marked as yours so that no later mail replaces it without asking. `ingest` runs the wiki step on a record that was saved before the wiki existed; `resolve review` walks through `Wiki\Review.md` one item at a time.
