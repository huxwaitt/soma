...
- OneDrive "Files On-Demand": a vault whose files are not downloaded locally makes `vault_find` / `vault_list` slow (every read pulls the file) and can make Obsidian show stale notes. Mark the vault folder "Always keep on this device".
