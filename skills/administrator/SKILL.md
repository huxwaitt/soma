...
| "here are my notes from the supplier call", "notes from today's sync", "write up the minutes" | `meetings` (`/administrator:notes [event words \| path] <raw notes>`) | Appends the raw notes to the meeting note, pulls out action items and waiting-on items (also into `Follow-ups.md`), sets `status: held`, updates `last_contact` on attendees, offers a minutes email that goes to Drafts (`outlook_send_mail(save_only=true)`) only after a yes. |
...
Load `inbox/SKILL.md`, `save/SKILL.md`, `meetings/SKILL.md` (plus `meetings/references/meeting-note.md`), or `schedule/SKILL.md` (plus `schedule/references/preferences.md`) when the workflow starts. Load the reference files below the first time you need them in a session.
...
4. On first use in a session, make sure `Administrator/`, `Administrator/Daily/`, `Administrator/Emails/`, `Administrator/Meetings/`, `Administrator/People/`, `Administrator/Attachments/` exist (create the missing ones) and that `Administrator/Follow-ups.md` exists (create it from the template in `references/vault.md` if it does not).
...
- Meeting notes are identified by `occurrence_key` (`global_id|<start ISO>`, on every event from `outlook_list_events`, `outlook_get_event`, `outlook_get_event_by_key`; fall back to building it from `global_id` + `start`, and to `entry_id` when `global_id` is empty). Before writing one, Grep `Administrator/Meetings/` frontmatter for `occurrence_key: "<key>"`; a hit means append a `## Update <ISO timestamp>` section, never a second note. `global_id` alone finds earlier occurrences of a recurring meeting and a meeting that was moved. The only frontmatter key edited in place is `status`. `prep`, `notes` and `schedule` all write the same note format.
- `Preferences.md` is identified by its path. The plugin creates it once from the template and never edits it.
...
- Sending plain mail is out of scope: never call `outlook_send_mail` without `save_only=true`, `outlook_reply_mail`, `outlook_forward_mail`, `outlook_delete_event`, or `outlook_respond_event` with `send_response=true`. The one allowed `outlook_send_mail` call is with `save_only=true`, from the `meetings` skill (minutes) or the `schedule` skill (proposed times), after the draft was shown and the user said yes; it lands in Drafts. If the user asks to send, say the plugin only saves to Drafts and they can send from Outlook. The only things that reach other people are a meeting invite (`outlook_create_event` with attendees) and a meeting update (`outlook_update_event`), both only from the `schedule` skill after a yes.
