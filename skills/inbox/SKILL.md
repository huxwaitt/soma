...
Vault conventions live in the core `soma` skill and `soma/references/vault.md` ("Workflow helpers" section); Outlook mechanics in the `outlook` skill and `soma/references/outlook.md`. Label rules: `references/labels.md`. A full run, call by call: `references/examples.md` — load it the first time you run this workflow in a session, not before.
...
- A working vault: `vault_status` once per session; if `soma_dir_exists` or any folder or file flag is false (including `Rules.md`), call `vault_init(created_by="soma/0.4.2")` and mention `/soma:setup`. Vault unset or not a directory: stop and tell the user; do not guess a path.
...
- **0 mails:** `vault_write_daily(date, labels=[], items=[], since, inbox_checked, created_by="soma/0.4.2")` still runs so the window moves on; tell the user "nothing new since <since>" and stop. No batch offer.
...
vault_write_daily(date=<today>, labels=<the JSON from step 4>, since=<since>, inbox_checked=<time of step 2>,
    ...
    tokens_used=<the turn's token count when the host shows one, else omit>, created_by="soma/0.4.2")
```
...
```
vault_row(action="append", path="Soma/Rules.md", section="Labels", row=[<match>, <field: from | domain | name | subject>, <label>])
```
...
- `vault_*` tools missing → vault server not running: point to `/soma:setup`, write nothing. `outlook_*` missing → the `outlook` skill's setup notes; no daily note.
