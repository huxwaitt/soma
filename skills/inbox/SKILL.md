...
- A working vault: `vault_status` once per session; if `soma_dir_exists` or any folder or file flag is false (including `Rules.md`), call `vault_init(created_by="soma/0.4.1")` and mention `/soma:setup`. Vault unset or not a directory: stop and tell the user; do not guess a path.
...
- **0 mails:** `vault_write_daily(date, labels=[], items=[], since, inbox_checked, created_by="soma/0.4.1")` still runs so the window moves on; tell the user "nothing new since <since>" and stop. No batch offer.
...
vault_write_daily(date=<today>, labels=<the JSON from step 4>, since=<since>, inbox_checked=<time of step 2>,
    ...
    tokens_used=<the turn's token count when the host shows one, else omit>, created_by="soma/0.4.1")
```
