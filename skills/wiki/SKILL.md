...
4. One call: `vault_wiki_write(record_path=<path>, pages=[{path | new: {type, title, aliases, lead, summary}, ops: [...]}], created_by="soma/0.4.1")`. `src` and `since` default to the record's id and date (its `internet_message_id`, `occurrence_key`, or a chat's `<chat_id>|<date>`). An empty op list is fine: code still writes the Records and History lines. Back come `applied` and `refused` per page (refusals are answers, not errors), `candidate` and `confirmed_decisions`; the record gets its `wiki:` key.
