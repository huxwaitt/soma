...
## Read step (prep, find, draft)

`vault_wiki_match` first, then `vault_wiki_read(path, sections=["lead","facts"], max_chars=800)` on the hits that matter. Show the lead before searching Outlook; quote facts with their page link. Never read `Index.md` or `Log.md` whole — `vault_wiki_match` and `vault_wiki_log(since, page, limit)` return slices.
