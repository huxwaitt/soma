...
5. **Second pass, same turn.** With the record still in front of you, answer one question in your head: *which facts in the record are not on the pages yet?* Read the ops you just sent (and the `refused` ones) against the record and list what is missing — a date, a name, a number, a promise. A non-empty list is a second, smaller `vault_wiki_ingest` with the same `record_path` and only those ops, sent before you reply. Never ask yourself whether the first ingest was good; only what it left out. Run this pass every time in `collect-information` and in `load-history` (every batch, every record); in `save` and `notes` only when the record is over 1500 characters. Nothing missing → say nothing about it.
6. A candidate over the threshold with no page (`candidates` from the match, or `candidate.over_threshold` in the ingest result): propose the page in the same turn ("Create topic `q3-budget` from these 2 records?"); create it with `vault_wiki_create(type, title, aliases, lead, summary, facts=[{text, since, src}], src, created_by, extra={email | domains | owner, org, due | decided, by})` only on a yes, or right away when the user named the subject.
...
| `conflicts-with` (with `id`, `current`, `since`) | Your `add` names a day or an amount, and a fact already on the page names a different one for the same thing — the page would hold both as true. Only that op was dropped; the rest of the call went through. | Decide which one holds. Resend the one op, not the whole call. The record is the newer → resend as `{"op": "supersede", "id": <the id from the refusal>, "text": <your text>, "since": …, "src": …}`. The record is older, same-day, or you are unsure → `{"op": "contest", "id": …, "text": …, "src": …}`, which puts both sides in Review. Say in one line which you chose. Two genuinely different things (the refusal is a rule, not a reading) → `contest` as well, and the Review line sorts it out. |
...
A fact whose line ends **`(one source, unconfirmed since <date>)`** rests on one kind of source and nothing has confirmed it since that day. Never state it flat: say where it stands ("one mail from January says the deadline is 29 August — worth checking"), or ask the user, or read the record it came from. In a draft, hedge it or leave it out. In the list form the same thing is `streams: 1` with a large `confirmed`.
