...
1. Load the `administrator` skill, then the `schedule` skill. Load the `outlook` skill if it is not already loaded. Load `skills/schedule/references/examples.md` only if a step is unclear.
2. Once per session: `vault_status` (if `files["Preferences.md"]` or anything else is false, `vault_init(created_by="administrator/0.1.0")` creates it with defaults — say so and mention `/administrator:setup` for other work hours), `outlook_whoami(response_format="json")`, and `vault_read("Administrator/Preferences.md")`. Do not read the preferences again in this session unless the user says they changed them.
...
9. Stop. This command writes no notes, creates no events and saves no drafts. If the user then says "book 2", run `/administrator:schedule` from step 5 of that command with the chosen candidate.
