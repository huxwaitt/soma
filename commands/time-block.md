...
11. **Report.** Three or four lines: blocks booked with focus and admin hours and the slack share kept (`totals.slack_share_kept`), existing blocks kept and days skipped; last week in one line; the note path with `action` and `obsidian://open?vault=<vault_name>&file=Administrator%2FTime-blocks%2F<week>`; when step 3 wrote `Priorities.md`, `obsidian://open?vault=<vault_name>&file=Administrator%2FPriorities.md` (edit it in Obsidian any time); when step 6 asked for peak hours, `obsidian://open?vault=<vault_name>&file=Administrator%2FPreferences.md` with "put `peak_hours: ["HH:MM-HH:MM"]` in the file".
