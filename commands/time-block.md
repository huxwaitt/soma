...
9. **Note.** `vault_time_block_write(week=<week>, blocks=<every block of the plan, the new ones with entry_id and occurrence_key merged in>, created_by="administrator/0.4.0")` → `{path, action, week, blocks, planned}`. `appended` = the week's note existed; the new rows sit under `## Update`. No `vault_write` for this note.
