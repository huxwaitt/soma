...
9. **Note.** `vault_time_block(action="write", week=<week>, blocks=<every block of the plan, the new ones with entry_id and occurrence_key merged in>, created_by="soma/0.4.1")` → `{path, action, week, blocks, planned}`. `appended` = the week's note existed; the new rows sit under `## Update`. No `vault_write` for this note.
