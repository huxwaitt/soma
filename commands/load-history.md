...
5. **The window.** `vault_load_history(action="next")` → `{batch_no, source, since, until, expected, skip_ids, list_with, reissued}`. Make the call in `list_with` as it stands (`outlook_list_mails(folder="inbox" | "sent", since, until, limit=100, preview_chars=80, fields=[…])` or `teams_list_chats(since, until, include_messages=true, per_chat=20, max_chars=300, limit=15)`), adding only `response_format="json"` on the mail call. Then turn the list oldest first, drop every id in `skip_ids` and every plainly automated mail without a call, and work on the first `expected`. `reissued: true` = the same window as last time, nothing new started.
...
10. If the host shows the turn's token count, add it as the last line; otherwise say nothing about it. (This command writes no daily note, so there is no `vault_write_daily(tokens_used=…)` call here.)
