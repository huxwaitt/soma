...
1. Load the `soma` skill, then the `load-history` skill and its `skills/load-history/references/examples.md`, then the `collect-information` skill and the `wiki` skill (plus `skills/wiki/references/examples.md` on the first ingest of a session). Load the `outlook` skill if it is not already loaded.
...
/soma:load-history all
/soma:load-history all 500k
```
