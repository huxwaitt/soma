...
2. Call `vault_status` (run `vault_init(created_by="soma/0.4.1")` if anything is missing) and `outlook_whoami(response_format="json")` for local time. Work out the ISO week and next week's Monday to Friday.
