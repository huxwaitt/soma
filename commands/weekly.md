...
6. Wiki (load the `wiki` skill): `vault_wiki_lint(fix=true, items=true)` then `vault_wiki_review(action="list")`. List the open Review items, ask one question per topic proposal and possible duplicate (create / merge only on a yes), and offer to ingest the records that were never ingested, ten at a time (the `wiki` skill's ingest steps on each, on a yes). Skip on "without wiki".
