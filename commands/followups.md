...
2. Call `vault_status` (run `vault_init(created_by="administrator/0.4.1")` if anything is missing) and `outlook_whoami(response_format="json")` for the user's own address(es) and local time.
