...
2. `vault_status` if not done yet this session. Vault unset or not a directory: stop and tell the user how to set `SOMA_VAULT` (see README) or to run `/soma:setup`. Any folder or file flag false: `vault_init(created_by="soma/0.4.1")`. `outlook_whoami(response_format="json")` once for the local time.
