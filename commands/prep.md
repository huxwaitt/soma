...
2. `vault_status` if not done yet this session (any folder or file flag false → `vault_init(created_by="administrator/0.3.0")`); `outlook_whoami(response_format="json")` once for the user's own address.
...
4. Per event, as the `meetings` skill describes:
   - `vault_prep_context(occurrence_key, global_id, attendees=[{name, address}…], subject=<event subject>)` — one call gives `existing_note`, `previous_occurrence.open_actions` (carried over), `people[]` (person pages, `company`, `last_contact`), `followups_open[]` and `wiki[]` (`path, type, title, status, lead, open[], facts[]` for the attendees' person pages and up to 3 topic pages matched on the subject). Do not `vault_find` / `vault_read` / `vault_wiki_read` for any of that; `wiki[]` becomes a `### Wiki` block in the Prep and the suggested points start from the topic lead and its open items.
