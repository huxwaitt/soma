...
2. `vault_status` if not done yet this session (any folder or file flag false → `vault_init(created_by="administrator/0.2.0")`); `outlook_whoami(response_format="json")` once for the user's own address.
