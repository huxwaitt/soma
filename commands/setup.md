---
description: Check that Outlook and the vault are reachable, create the Administrator folder, the wiki and their files if they are missing, set your work hours once, and offer to move an older vault's People folder into the wiki (dry run shown first). Run this first on a new machine.
argument-hint: ""
---
...
8. **Report.** Six lines at most: servers (both present or which one is missing), Outlook account and timezone, vault path and name, what was created, the migration result or offer, the sandbox warning if any. End with the link `obsidian://open?vault=<vault_name>&file=Administrator%2FPreferences.md` (`vault_name` from `vault_status`) and one line: "Edit Preferences.md in Obsidian any time; the plugin reads it once per session and never changes it."
...
> Vault: `C:\Users\<you>\Documents\Vault` (name `Vault`). Created `Administrator/`, 7 folders, `Wiki/` (Wiki.md, Index, Log, Review, 4 page folders), `Follow-ups.md`, `Preferences.md` (09:00–17:00, buffer 15) and 5 views.
