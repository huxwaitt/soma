...
2. `vault_status` once per session; `folders["Wiki"]` false → `vault_init(created_by="soma/0.4.1")` and mention `/soma:setup` (it offers the migration of an older vault).
...
6. **Review.** `vault_wiki_keep(action="review")` → `{open: [{n, text}], done}`; show the open lines numbered. For each answer the user gives: `vault_wiki_keep(action="review", review_action="resolve", item="<n>", resolution_ops=[...], created_by="soma/0.4.1")` (a `confirm` with `src: user` on the fact that holds, a `supersede` with `src: user` when the other text is right; `{"op": "confirm", "id": <the first fact>}` for an "unconfirmed decision" line, or `{"op": "status", "value": "dropped"}` to drop it; `done` / `reschedule` for an "overdue" line; no ops when the answer needs none, e.g. "leave it dormant"). A merge yes → `vault_wiki_keep(action="merge", keep, drop, created_by="soma/0.4.1")`, which moves the line to Done itself. One item per question; never resolve without an answer.
