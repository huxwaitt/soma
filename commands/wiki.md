...
4. **Statement.** Match the page as in step 3b (ask when unclear), `vault_wiki_read(path, sections=["facts"])` (add `"open"` for a commitment), pick the op (`add` / `update` / `supersede` / `retire` / `status` / `alias` / `lead`; `open` / `done` / `reschedule` for a commitment, with `owner` and `due` when the user named them; `due` / `outcome` / `milestone` / `risk` / `link` on a topic), show it in one line ("Supersede `Deadline … 2026-08-27` → `Deadline … 2026-08-29` on Topics/q3-budget, src: user — ok?") and on a yes: `vault_wiki_apply(path, ops=[...], created_by="administrator/0.4.0")` (`src` defaults to `user`). `since` = today unless the user gave a date. A `note:` statement is refused: `## Notes` is the user's section in Obsidian, the plugin never writes it. A decision the user states is `vault_wiki_create(type="decision", …, extra={"decided": …, "by": [...]})`, and the Review line it writes is resolved in the same turn (`resolution_ops=[{"op": "confirm", "id": <the first fact>}]`), because the user just said it. On a decision page every rewriting op comes back `append-only`: say so and offer a new decision or a fact on the topic.
...
6. **Review.** `vault_wiki_review(action="list")` → `{open: [{n, text}], done}`; show the open lines numbered. For each answer the user gives: `vault_wiki_review(action="resolve", item="<n>", resolution_ops=[...], created_by="administrator/0.4.0")` (a `confirm` with `src: user` on the fact that holds, a `supersede` with `src: user` when the other text is right; `{"op": "confirm", "id": <the first fact>}` for an "unconfirmed decision" line, or `{"op": "status", "value": "dropped"}` to drop it; `done` / `reschedule` for an "overdue" line; no ops when the answer needs none, e.g. "leave it dormant"). A merge yes → `vault_wiki_merge(keep, drop, created_by="administrator/0.4.0")`, which moves the line to Done itself. One item per question; never resolve without an answer.
