...
7. Write a summary (at most 2 sentences) and the action items from `body_trimmed`, then one call: `vault_save_email(mail=<step 4 JSON>, summary, action_items, attachments_saved=[<paths>], msg_file=<path>, self_addresses=<whoami addresses>, company=<step 5>, created_by="administrator/0.4.0")`. It writes the email note (`Administrator/Emails/YYYY-MM-DD <slug>.md`, or an `## Update` on an existing one), the sender's person page and, for `waiting`, one open item owned by the counterpart on their page (`Follow-ups.md` is written from those items). No `vault_write`, `vault_append_row` or hand-written frontmatter.
