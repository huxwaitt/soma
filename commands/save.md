...
2. Once per session: `vault_status` (if `administrator_dir_exists` or any folder or file flag is false, `vault_init(created_by="administrator/0.2.0")` and mention `/administrator:setup`) and `outlook_whoami(response_format="json")` for the user's own addresses.
3. Find the mail:
