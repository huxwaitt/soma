...
1. Load the `soma` skill, then the `save` skill and its `skills/save/references/examples.md`. Load the `outlook` skill if it is not already loaded.
2. Once per session: `vault_status` (if `soma_dir_exists` or any folder or file flag is false, `vault_init(created_by="soma/0.4.2")` and mention `/soma:setup`) and `outlook_whoami(response_format="json")` for the user's own addresses.
