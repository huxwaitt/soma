...
9. Wiki ingest, unless the user said "save without wiki": load the `wiki` skill, `vault_wiki_match(text=<subject + first 300 chars of the notes>, people=[attendee addresses], domains=[their domains])`, `vault_wiki_read(path, sections=["lead","facts"])` on at most 3 hits, one `vault_wiki_ingest(record_path=<meeting path>, pages=[...], created_by="administrator/0.4.0")`. Decisions become `add` / `supersede` facts on the topic page with `since` = meeting date; attendees get `role` / `confirm`; items owned by others become `open` items. A topic seen in 2+ records on 2+ days with no page → propose it, create only on a yes.
