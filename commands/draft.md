...
2. Call `vault_status` (run `vault_init(created_by="administrator/0.2.0")` if anything is missing) and `outlook_whoami(response_format="json")` for your own address(es) and local time.
...
5. `vault_wiki_match(text=<subject + 300 chars>, people=[<sender>], limit=4)` then `vault_wiki_read(path, sections=["lead","facts","open","notes"], max_chars=800)` on the sender's person page and at most one topic page (how they work with you, a `Voice with this person:` block under `## Notes`, the facts the reply may state); `## Open` rows in `Follow-ups.md` for this person or subject; `vault_find("email", …, fields=["status"])` for an existing note; `## Voice` in `Preferences.md`.
