...
2. Call `vault_status` (run `vault_init(created_by="soma/0.4.2")` if anything is missing) and `outlook_whoami(response_format="json")` for your own address(es) and local time.
