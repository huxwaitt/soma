...
5. `vault_wiki_search(query=<the sender's name + the subject>, brief=true, max_chars=1000)` (how they work with you, the facts the reply may state) and one `vault_wiki_read(path=<the sender's page>, sections=["notes"], max_chars=600)` for a `Voice with this person:` block, which search never reads; `## Open` rows in `Follow-ups.md` for this person or subject; `vault_find("email", …, fields=["status"])` for an existing note; `## Voice` in `Preferences.md`.
