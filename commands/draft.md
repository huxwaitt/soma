...
2. Call `vault_status` (run `vault_init(created_by="administrator/0.1.0")` if anything is missing) and `outlook_whoami(response_format="json")` for your own address(es) and local time.
...
5. `vault_find("person", {"email": <sender>})` + `vault_read` when found; `## Open` rows in `Follow-ups.md` for this person or subject; `vault_find("email", …, fields=["status"])` for an existing note; `## Voice` in `Preferences.md`.
