...
2. `vault_status` if not done yet this session; any folder or file flag false → `vault_init(created_by="soma/0.4.2")` and mention `/soma:setup`. `outlook_whoami(response_format="json")` once for the local time.
3. Inbox, as `/soma:inbox` steps 3 to 5: `outlook_list_mails(... fields=["entry_id", "internet_message_id", "from_address", "from", "subject", "received", "preview"], preview_chars=80 ...)`, `vault_inbox_prepare(items, date=<date>)`, then label only the `label: null` entries into one JSON list `[{entry_id, label, reason}]` (`outlook_get_mail(..., fields=["subject", "body_trimmed"], trim_quoted=true)` for at most 5). Hold the batch offer for step 8.
...
> Today: 09:30 Stand-up (Teams), 10:15 [Focus] ACME supplier contract, 12:00 [Admin] Email and small tasks, 13:00 Budget review with Jane (Room 4). Watch out: clash 13:00–14:00 with Dentist; no prep note for the budget review — run /soma:prep? Stand-up moved to 10:30–11:00 and lands on the focus block — move the block to 14:15–15:45? Move it?
