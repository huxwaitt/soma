...
2. `vault_status` if not done yet this session; any folder or file flag false → `vault_init(created_by="administrator/0.4.0")` and mention `/administrator:setup`. `outlook_whoami(response_format="json")` once for the local time.
