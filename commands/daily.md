...
2. `vault_status` if not done yet this session; any folder or file flag false → `vault_init(created_by="soma/0.4.1")` and mention `/soma:setup`. `outlook_whoami(response_format="json")` once for the local time.
